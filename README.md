...
**Agenten je Projekt.** Jedes Projekt bekommt eigene OpenClaw-Agenten je Schritt — `agents.entries.pipeline-<projekt>-<step>` —,
angelegt vom Plugin beim ersten Lauf (`openclaw agents add`, Workspace `workspace-pipeline-<projekt>-<step>`, SOUL.md aus der
Vorlage kopiert). Die globalen `pipeline-<step>` sind nur noch die **Vorlagen**. Was das Projekt abweichend will, steht in
`<repo>/.agentops/agents.json` — `{ "plan": { "model": "anthropic/claude-opus-5", "thinking": "high", "tools": ["read", "write", "exec"] }, … }`:
Modell, Effort (OpenClaws Thinking-Level: off, minimal, low, medium, high, xhigh, adaptive, max, ultra) und Tool-Liste
(absolute Allowlist der Kern-Tools, Kennungen aus `tools.catalog`; leer = Tools der Vorlage). Das Cockpit schreibt und
committet die Datei (`PUT /api/projects/{name}/agents/{step}` mit `model`, `thinking`, `tools`; leerer Wert = Vorlage) und
stößt den Abgleich an (`POST …/agents/sync`); das Plugin gleicht die Projekt-Agenten außerdem vor jedem Schritt mit Vorlage
und Datei ab (`config get agents`, bei Abweichung ein `config patch` mit Hot-Reload) und startet den Lauf als Session
des Projekt-Agenten. Sackgassen, die das entschieden haben: `sessions.create` kennt `toolOverrides` nur für MCP-Server,
Skills und Websuche, nicht für Kern-Tools; `subagent.run` kennt weder Tools noch Thinking; die Gateway-API steht nur
gebündelten Plugins offen. Tools und Effort gibt es in OpenClaw nur je Agent — also gibt es Agenten je Projekt. Dafür muss das Plugin
