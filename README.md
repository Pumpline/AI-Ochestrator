...
**Agenten je Projekt.** `<repo>/.agentops/agents.json` — `{ "plan": { "model": "anthropic/claude-opus-5", "thinking": "high" }, … }` — setzt Modell
und Effort (OpenClaws Thinking-Level: off, minimal, low, medium, high, xhigh, adaptive, max, ultra) je Schritt für dieses Projekt; das Cockpit schreibt und committet die Datei (`PUT/DELETE /api/projects/{name}/agents/{step}`),
das Plugin liest sie beim Start jedes Schritts, gibt `provider`/`model` an `subagent.run` mit und legt die Session des Laufs
vorab mit `thinkingLevel` an (`openclaw gateway call sessions.create`, weil `api.runtime.gateway.request` gebündelten Plugins
vorbehalten ist; `subagent.run` selbst kennt kein Thinking, und die Direktive `/think` in der Nachricht greift dort nicht). Dafür muss das Plugin
